...
def index(request) :
    # all : select 문
    # order_by : order by문. -를 붙이면 내림차순 정렬
    topic_list = models.TopicModel.objects.all().order_by('-topic_idx')
    # print(topic_list)
    # print(topic_list.query)
    # print(topic_list.values())

    # test = models.ContentModel.objects.all().values('content_topic_idx').\
    #     annotate(cnt=Count('content_text'))
    # for obj in test :
    #     print(test)

    ## 외래키 관계인 경우 : 모델 변수의 이름을 넣어주면 값을 가져오고
    # 모델 변수 이름__ 를 하면 외래키 관계로 참조하고 있는 모델의 데이터를 가져올 수 있다.
